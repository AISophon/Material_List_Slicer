...
# 配对任务
def get_item_quantity(df, row_index):
    """获取指定行的物品种类和数量，并将数量转换为整数。"""
    kind = df.iloc[row_index, 0]  # 物品种类
    quantity = df.iloc[row_index, 1]  # 物品数量
    quantity = int(quantity)  # 强制转换 np.int64 为 int
    return kind, quantity
...
def pair_tasks(df):
    ...
    # 获取物品种类和数量
    kind, quantity = get_item_quantity(df, row_index)
